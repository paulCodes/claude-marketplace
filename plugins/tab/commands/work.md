...
| **Verification** | `/tab-workflow:verify` (auto-detect) |
...
- **NEVER** run tests, linters, or typecheckers directly. Delegate to agents or `/tab-workflow:verify`.
...
If no Tab project exists yet, suggest running the **tab-workflow:brainstorm** skill first.
...
- **No tasks**: Use **tab-workflow:brainstorm** skill to create them, or work with user to break the design into tasks
...
**Every time code changes, invoke the `tab-workflow:verify` skill.** It will:
...
```
Implement --> /tab-workflow:verify --> Test --> /tab-workflow:verify --> Review --> Fix --> /tab-workflow:verify --> Commit
```
...
2. **Run `/tab-workflow:verify`.** Mark task `done` in Tab if verification passes.
...
**Run the full commit gate:** Invoke `/tab-workflow:verify --commit-gate` for the project. This checks both technical (lint, typecheck, tests) and workflow (review findings resolved, QA findings resolved, all tasks done, no governance items) gates in one pass.
...
- **Tab brainstorming**: tab-workflow:brainstorm skill (idea --> design --> Tab project + tasks)
- **Verification**: tab-workflow:verify skill (auto-detect project type, run checks, create bug tasks)
