---
name: verify
description: "Run verification checks (lint, typecheck, tests), create Tab bug tasks for any failures, and dispatch sub-agents to fix them. Use anytime with /tab-workflow:verify to check code health and auto-triage failures into Tab."
---

# Tab Verify

Run project verification checks, create Tab tasks for failures, and dispatch fix sub-agents. Works standalone or as part of the tab-workflow:work workflow.

## When to use

- `/tab-workflow:verify` — run checks anytime
- After any code change during tab-workflow:work
- Before committing
...
When invoked with `--commit-gate` or called from tab-workflow:work Step 5, run the FULL pre-commit checklist. This goes beyond lint/typecheck/tests to verify the entire workflow completed correctly.
...
## Integration with tab-workflow:work

tab-workflow:work invokes tab-workflow:verify at these points:
...
When called without `--commit-gate`, tab-workflow:verify runs technical checks only (lint, typecheck, tests). The commit gate adds workflow checks (review findings, QA findings, task statuses).

## Standalone usage

When invoked standalone (not part of tab-workflow:work):
...
```
/tab-workflow:verify                    # run all checks
/tab-workflow:verify --commit-gate      # full pre-commit checklist (technical + workflow)
/tab-workflow:verify --lint-only        # just lint
/tab-workflow:verify --tests-only       # just tests
/tab-workflow:verify --no-fix           # report only, don't dispatch fix agents
```
