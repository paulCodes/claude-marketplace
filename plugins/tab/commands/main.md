...
- **PR review intent** (PR URL, "review PR #123", "pr dashboard") → `Skill("tab-workflow:review-pr")` with the user's args. Skip all other steps.
...
**If no match found:**
- User described an idea → `Skill("tab-workflow:brainstorm")` with the user's message. Skip Steps 3-4.
...
| Intent | Route to |
|--------|----------|
| New idea, "I want to build X" | `Skill("tab-workflow:brainstorm")` |
| "refine", "groom", "review tasks" | `Skill("tab-workflow:refine")` |
| "work on X", "implement", "tackle" | `Skill("tab-workflow:work")` |
| "continue", "resume", "pick up" | `Skill("tab-workflow:work")` |
...
| "review PR", "review #3", PR URL | `Skill("tab-workflow:review-pr")` |
| "pr dashboard", "open PRs", "show PRs" | `Skill("tab-workflow:review-pr")` |
...
If status shows many tasks with missing `implementation` or `acceptance_criteria`, suggest running `tab-workflow:refine` before starting work. This is a suggestion, not an automatic route — let the user decide.
