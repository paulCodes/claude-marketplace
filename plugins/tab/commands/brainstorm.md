...
| **Personal** | `~/workspaces/{project-slug}` | `{project-slug}-{kebab}` | Short descriptive message | `/tab-workflow:verify` (auto-detect) |
...
13. **Transition to implementation** — invoke the **tab-workflow:work** skill to execute the project
...
This makes the documents available when tab-workflow:work loads the project — sub-agents get them as context automatically.
...
Explore context → Create/load draft project (crash recovery) → Visual companion?
  ...
  → Create tasks → Write implementation details → User reviews → tab-workflow:work
...
- Invoke the **tab-workflow:work** skill to execute the project
- tab-workflow:work handles: research → branch → implement → verify → review → fix → commit
- tab-workflow:work uses **tab-workflow:verify** for automated test/lint/typecheck with bug task creation
